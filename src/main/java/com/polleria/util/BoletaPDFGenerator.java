...
public class BoletaPDFGenerator {
    ...
    public static byte[] generar(Pedido pedido, List<DetallePedido> detalles,
                                  Pago pago, Usuario usuario) throws Exception {
        ...
        Paragraph pNro = new Paragraph("N B001-" + String.format("%06d", pedido.getId()), fBold);
        ...
        for (DetallePedido d : detalles) {
            ...
            if (d.getOpciones() != null && !d.getOpciones().isEmpty()) {
                ...
                for (DetallePedidoOpcion op : d.getOpciones()) {
                    if (sb.length() > 0) sb.append(", ");
                    sb.append(op.getNombreOpcion());
                }
                cNom.addElement(new Paragraph(sb.toString(), fSmall));
            }
        ...
        cTL.setBorder(Rectangle.TOP); cTL.setPadding(6);
        PdfPCell cTV = new PdfPCell(new Phrase("S/ " + String.format("%.2f", pedido.getTotal()), fTotal));
        cTV.setBorder(Rectangle.TOP); cTV.setPadding(6);
    ...
    private static void filaCliente(PdfPTable t, String label, String val, Font fL, Font fV) {
        ...
        PdfPCell c2 = new PdfPCell(new Phrase(val != null ? val : "-", fV));
        c2.setBorder(Rectangle.BOTTOM); c2.setBorderColor(borde); c2.setPadding(5);
        t.addCell(c1); t.addCell(c2);
    }

    private static void filaTotal(PdfPTable t, String label, String val, Font fL, Font fV) {
        PdfPCell c1 = new PdfPCell(new Phrase(label, fL));
    ...
    private static PdfPCell celda(String txt, Font f, BaseColor bg, int align) {
