...
<header class="w-full shadow">
    ...
    <div class="w-full flex items-center justify-between px-4 md:px-10 py-2 gap-4">
        ...
        <a href="${pageContext.request.contextPath}/home" class="flex-shrink-0">
            <img src="${pageContext.request.contextPath}/img/logoheader.webp"
        ...
        <div class="flex gap-3 text-xl items-center">
            ...
            <c:choose>
                <c:when test="${not empty sessionScope.usuario}">
                    <c:if test="${sessionScope.rolNombre == 'CLIENTE'}">
                        <div class="relative hidden md:block" id="clienteMenuDesktop">
                            <button onclick="toggleClienteMenuDesktop()"
                                ...
                                <i class="fa-solid fa-chevron-down text-xs text-gray-400" id="iconClienteMenuDesktop"></i>
                            </button>
                            <div id="dropdownClienteDesktop"
                ...
                <c:otherwise>
                    <a href="${pageContext.request.contextPath}/login"
                       class="hidden md:block hover:text-red-500 transition">
                        <ion-icon name="person-outline"></ion-icon>
                    </a>
                </c:otherwise>
            </c:choose>

            <!-- ICONO USUARIO MÓVIL — siempre visible -->
            <div class="relative md:hidden" id="clienteMenuMovilIcono">
                <c:choose>
                    <c:when test="${not empty sessionScope.usuario}">
                        <!-- Ícono rojo si está logueado -->
                        <button onclick="toggleClienteMenuMovil()"
                                class="text-red-500 text-2xl relative">
                            <ion-icon name="person-circle-outline"></ion-icon>
                            <!-- Punto indicador sesión activa -->
                            <span class="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-white"></span>
                        </button>
                    </c:when>
                    <c:otherwise>
                        <!-- Ícono normal si no está logueado -->
                        <a href="${pageContext.request.contextPath}/login"
                ...
                <!-- Dropdown móvil usuario -->
                <c:if test="${not empty sessionScope.usuario}">
                    <div id="dropdownClienteMovil"
                         class="hidden absolute right-0 top-full mt-2 w-64 bg-white rounded-2xl shadow-xl border border-gray-100 z-50 overflow-hidden">
                        <div class="px-4 py-3 border-b border-gray-100 bg-red-50">
                            <p class="text-sm font-semibold text-gray-800">${sessionScope.usuario.nombre}</p>
                            <p class="text-xs text-gray-500">${sessionScope.usuario.email}</p>
                        </div>
            ...
            <!-- Corazón y carrito — siempre visibles -->
            <a href="#" class="hover:text-red-500 transition text-2xl">
                <ion-icon name="heart-outline"></ion-icon>
            </a>
            <a href="${pageContext.request.contextPath}/carrito"
               class="hover:text-red-500 transition text-2xl">
    ...
    <div id="buscadorMovil" class="hidden px-4 pb-3 md:hidden">
        <div class="flex items-center border rounded-full px-4 py-2 bg-gray-50">
            <input type="text" id="buscadorMovilInput"
                   class="w-full outline-none text-sm bg-transparent"
                   placeholder="Buscar productos..."
                   onkeydown="if(event.key==='Enter') buscarMovil()">
            <i class="fa-solid fa-magnifying-glass cursor-pointer text-gray-400"
               onclick="buscarMovil()"></i>
        </div>
    </div>
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                <div class="bg-green-500 px-6 py-5 text-center">
                    ...
                    <p class="text-green-100 text-sm mt-1">
                        Pedido <strong>#<span id="modalPedidoIdGlobal">-</span></strong>
                    </p>
                </div>
                <div class="px-6 py-5 text-center">
                    ...
                    <div class="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 mb-5 flex items-start gap-3 text-left">
                        ...
                        <p class="text-xs text-blue-700">
                            Tu boleta en PDF ha sido enviada a tu correo. También puedes descargarla aquí.
                        </p>
                    </div>
    ...
    <script>
        // ── Buscador ──────────────────────────────────────────────────────────
        function buscar() {
            var t = document.getElementById('buscadorHeader').value.trim();
            if (t) window.location.href = '${pageContext.request.contextPath}/buscar?q=' + encodeURIComponent(t);
        }
        function buscarMovil() {
            var t = document.getElementById('buscadorMovilInput').value.trim();
            if (t) window.location.href = '${pageContext.request.contextPath}/buscar?q=' + encodeURIComponent(t);
        }

        // ── Menús ─────────────────────────────────────────────────────────────
        function toggleMenu() {
            document.getElementById('menuMovil').classList.toggle('hidden');
        }
        ...
        function toggleClienteMenuDesktop() {
            document.getElementById('dropdownClienteDesktop').classList.toggle('hidden');
            document.getElementById('iconClienteMenuDesktop').classList.toggle('rotate-180');
        }

        // ── Dropdown usuario MÓVIL ────────────────────────────────────────────
        function toggleClienteMenuMovil() {
            var dd = document.getElementById('dropdownClienteMovil');
        ...
        document.addEventListener('click', function(e) {
            // Desktop
            var menuDesktop = document.getElementById('clienteMenuDesktop');
            if (menuDesktop && !menuDesktop.contains(e.target)) {
                var dd = document.getElementById('dropdownClienteDesktop');
                if (dd) dd.classList.add('hidden');
                var ic = document.getElementById('iconClienteMenuDesktop');
                if (ic) ic.classList.remove('rotate-180');
            }
            // Móvil
            var menuMovil = document.getElementById('clienteMenuMovilIcono');
            if (menuMovil && !menuMovil.contains(e.target)) {
                var ddm = document.getElementById('dropdownClienteMovil');
